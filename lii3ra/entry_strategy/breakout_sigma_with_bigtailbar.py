...
class BreakoutSigmaWithBigTailBarFactory(EntryStrategyFactory):
    params = {
    ...
    def create(self, ohlcv, optimization=False):
        ...
        if not optimization:
            ...
            strategies.append(BreakoutSigmaWithBigTailBar(ohlcv
                                                          , long_bb_span
                                                          , long_bb_ratio
                                                          , short_bb_span
                                                          , short_bb_ratio
            ...
            for long_span in long_spans:
                for long_ratio in long_ratios:
                    for stop_order in stop_order_list:
                        strategies.append(BreakoutSigmaWithBigTailBar(ohlcv, long_span, long_ratio, 0, 0.0, stop_order))
            for short_span in short_spans:
                for short_ratio in short_ratios:
                    for stop_order in stop_order_list:
                        strategies.append(BreakoutSigmaWithBigTailBar(ohlcv, 0, 0, short_span, short_ratio, stop_order))
            for long_span in long_spans:
                for long_ratio in long_ratios:
                    for short_span in short_spans:
                        for short_ratio in short_ratios:
                            for stop_order in stop_order_list:
                                strategies.append(BreakoutSigmaWithBigTailBar(ohlcv, long_span, long_ratio
                                                                              , short_span, short_ratio, stop_order))
        return strategies


class BreakoutSigmaWithBigTailBar(EntryStrategy):
    """
    ...
    def check_entry_long(self, idx, last_exit_idx):
        ...
        bull_bear_tail = self._get_bull_bear_tail(idx)
        if long_flg and not short_flg and bull_bear_tail == 1:
    ...
    def check_entry_short(self, idx, last_exit_idx):
        ...
        bull_bear_tail = self._get_bull_bear_tail(idx)
        if not long_flg and short_flg and bull_bear_tail == 2:
    ...
    def _get_bull_bear_tail(self, idx):
        open_price = self.ohlcv.values['open'][idx]
        high = self.ohlcv.values['high'][idx]
        low = self.ohlcv.values['low'][idx]
        close = self.ohlcv.values['close'][idx]
        high_before = self.ohlcv.values['high'][idx - 1]
        low_before = self.ohlcv.values['low'][idx - 1]
        bull_tail_bar = close > open_price and (open_price - low) > (close - open_price) and high > high_before
        if bull_tail_bar:
            return 1
        bear_tail_bar = close < open_price and (high - open_price) > (open_price - close) and low < low_before
        if bear_tail_bar:
            return 2
        return 0
