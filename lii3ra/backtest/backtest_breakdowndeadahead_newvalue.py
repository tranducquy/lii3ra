...
from donkatsu.entry_strategy.breakdown_dead_ahead import BreakdownDeadAhead
...
def backtest_open_atrbasedbreakout_close_newvalue(symbol
    ...
    try:
        ...
        open_title = f"BreakdownDeadAhead[{atr_span:.0f},{length_of_trend:.0f}][{atr_mult:.2f}]"
        close_title = f"NewValue"
        open_strategy = BreakdownDeadAhead(open_title, ohlcv, atr, length_of_trend, atr_mult)
...
def backtest(symbol, ashi, start_date, end_date, brute_force=False):
    ...
    if brute_force:
        ...
        if symbol in params:
            ...
            length_of_trend = params[symbol[1]]
            atr_mult = params[symbol][2]
