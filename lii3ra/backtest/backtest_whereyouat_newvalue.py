...
from donkatsu.entry_strategy.where_you_at import WhereYouAt
...
def backtest_run(symbol
    ...
    try:
        ...
        open_title = f"WhereYouAt[{long_threshold:.2f}][{short_threshold:.2f}]"
        close_title = f"NewValue"
        open_strategy = WhereYouAt(open_title, ohlcv, long_threshold, short_threshold)
...
def backtest(symbol, ashi, start_date, end_date, brute_force=False, long_flg=False, short_flg=False):
    ...
    if brute_force:
        ...
        #symbolごとのparameter
        if symbol in params:
            long_threshold  = params[symbol][0]
            short_threshold = params[symbol][1]
        backtest_run(symbol
                        , ashi
                        , start_date
                        , end_date
