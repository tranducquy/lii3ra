...
from donkatsu.entry_strategy.newhigh_with_consecutive_highs import NewHighWithConsecutiveHighs
...
def backtest_run(symbol
    ...
    try:
        ...
        open_title = f"NewHigh[{long_lookback_period:.0f},{long_consecutive_cnt:.0f}][{short_lookback_period:.0f},{short_consecutive_cnt:.0f}]"
        close_title = f"Timed[{timed_method}][{num_of_bars_long:.0f}][{num_of_bars_short:.0f}][{close_losscut_ratio:.2f}]"
        open_strategy = NewHighWithConsecutiveHighs(open_title, ohlcv, long_lookback_period, long_consecutive_cnt, short_lookback_period, short_consecutive_cnt)
...
def backtest(symbol, ashi, start_date, end_date, brute_force=False):
    ...
    if brute_force:
        ...
        # Entry
        # long
        long_lookback_period = 10
        long_consecutive_cnt = 3
        # short1
        short_lookback_period = 10
        short_consecutive_cnt = 3
        # Exit
        timed_method        = 1
        num_of_bars_long    = 3
        num_of_bars_short   = 3
        ...
        #symbolごとのparameter
        if symbol in params:
            long_lookback_period  = params[symbol][0]
            long_consecutive_cnt  = params[symbol][1]
            short_lookback_period = params[symbol][2]
            short_consecutive_cnt = params[symbol][3]
            timed_method          = params[symbol][4]
            num_of_bars_long      = params[symbol][5]
            num_of_bars_short     = params[symbol][6]
            close_losscut_ratio   = params[symbol][7]
        backtest_run(symbol
                        , ashi
                        , start_date
                        , end_date
