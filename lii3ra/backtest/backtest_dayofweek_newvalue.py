...
from donkatsu.entry_strategy.day_of_week import DayOfWeek
...
def backtest_run(symbol
    ...
    try:
        ...
        open_title = f"DayOfWeek[{long_dayofweek:.0f},{short_dayofweek:.0f}]"
        close_title = f"NewValue"
        open_strategy = DayOfWeek(open_title, ohlcv, long_dayofweek, short_dayofweek)
...
def backtest(symbol, ashi, start_date, end_date, brute_force=False):
    ...
    if brute_force:
        ...
        # symbolごとのparameter
        if symbol in params:
            long_dayofweek = params[symbol][0]
            short_dayofweek = params[symbol][1]
        backtest_run(symbol
                     , ashi
                     , start_date
                     , end_date
