...
from donkatsu.entry_strategy.introducing_serial_correlation import IntroducingSerialCorrelation
...
def backtest_run(symbol
    ...
    try:
        ...
        open_title = f"IntroSerial[{lookback:.0f},{winner_waiting_period:.0f},{loser_waiting_period:.0f}]"
        close_title = f"NewValue"
        entry_strategy = IntroducingSerialCorrelation(open_title, ohlcv, lookback, winner_waiting_period, loser_waiting_period)
...
def backtest(symbol, ashi, start_date, end_date, brute_force=False):
    ...
    if brute_force:
        ...
        if symbol in params:
            ...
            winner_waiting_period = params[symbol][1]
            loser_waiting_period = params[symbol][2]
        backtest_run(symbol
                     , ashi
                     , start_date
                     , end_date
