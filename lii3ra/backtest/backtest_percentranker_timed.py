...
from donkatsu.entry_strategy.percent_ranker import PercentRanker
...
def backtest_run(symbol
                    ...
                    , long_percentile_span
                    , long_adx_span
                    , long_adx_ratio1
                    , long_adx_ratio2
                    , short_percentile_span
                    , short_adx_span
                    , short_adx_ratio1
                    , short_adx_ratio2
                    , timed_method
    ...
    try:
        ...
        open_title = f"PercentRanker[{percentile_lookback_span:.0f}][{long_percentile_span:.0f},{long_adx_span:.0f},{long_adx_ratio1:.2f},{long_adx_ratio2:.2f}][{short_percentile_span:.0f},{short_adx_span:.0f},{short_adx_ratio1:.2f},{short_adx_ratio2:.2f}]"
        close_title = f"Timed[{timed_method}][{num_of_bars_long:.0f}][{num_of_bars_short:.0f}][{close_losscut_ratio:.2f}]"
        open_strategy = PercentRanker(open_title, ohlcv, percentile_lookback_span, long_percentile_span, long_adx, long_adx_ratio1, long_adx_ratio2, short_percentile_span, short_adx, short_adx_ratio1, short_adx_ratio2)
...
def backtest(symbol, ashi, start_date, end_date, brute_force=False):
    ...
    if brute_force:
        ...
        #デフォルトのパラメータ
        # Entry
        percentile_lookback_span = 25
        # long
        long_percentile_ratio = 0.75
        long_adx_span = 14
        long_adx_ratio1 = 0.20
        long_adx_ratio2 = 0.30
        # short 
        short_percentile_ratio = 0.25
        short_adx_span = 14
        short_adx_ratio1 = 0.20
        short_adx_ratio2 = 0.30
        # Exit
        timed_method        = 1
        num_of_bars_long    = 3
        num_of_bars_short   = 3
        ...
        #symbolごとのparameter
        if symbol in params:
            percentile_lookback_span = params[symbol][0] 
            long_percentile_ratio    = params[symbol][1]
            long_adx_span            = params[symbol][2]
            long_adx_ratio1          = params[symbol][3]
            long_adx_ratio2          = params[symbol][4]
            short_percentile_ratio   = params[symbol][5]
            short_adx_span           = params[symbol][6]
            short_adx_ratio1         = params[symbol][7]
            short_adx_ratio2         = params[symbol][8]
            timed_method             = params[symbol][9]
            num_of_bars_long         = params[symbol][10]
            num_of_bars_short        = params[symbol][11]
            close_losscut_ratio      = params[symbol][12]
        backtest_run(symbol
                        , ashi
                        , start_date
                        , end_date
