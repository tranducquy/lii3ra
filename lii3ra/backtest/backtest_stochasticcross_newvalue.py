...
from donkatsu.technical_indicator.stochastics import Stochastic
from donkatsu.entry_strategy.stochastic_cross import StochasticCross
...
def backtest_run(symbol
    ...
    try:
        ...
        stoch = Stochastic(ohlcv, stoch_period, stoch_smoothing1, stoch_smoothing2)
        open_title = f"StochCross[{stoch_period:.0f},{stoch_smoothing1:.0f},{stoch_smoothing2:.0f}]"
        close_title = f"NewValue"
        open_strategy = StochasticCross(open_title, ohlcv, stoch)
...
def backtest(symbol, ashi, start_date, end_date, brute_force=False):
    ...
    if brute_force:
        ...
        # symbolごとのparameter
        if symbol in params:
            stoch_period     = params[symbol][0]
            stoch_smoothing1 = params[symbol][1]
            stoch_smoothing2 = params[symbol][2]
        backtest_run(symbol
                     , ashi
                     , start_date
                     , end_date
