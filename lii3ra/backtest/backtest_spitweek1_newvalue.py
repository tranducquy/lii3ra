...
from donkatsu.entry_strategy.split_week import SplitWeek
...
def backtest_run(symbol
    ...
    try:
        ...
        open_title = f"SplitWeek1[{atr_span:.0f},{atr_ratio:.0f},{maxl:.2f},{entry_dayofweek_title}]"
        close_title = f"NewValue"
        entry_strategy = SplitWeek(open_title, ohlcv, atr, atr_ratio, lookback, maxl, entry_dayofweek)
...
def backtest(symbol, ashi, start_date, end_date, brute_force=False):
    ...
    if brute_force:
        ...
        lookback = 15
        maxl = 2500
        entry_dayofweek = [1, 2, 3]
        # symbolごとのparameter
        if symbol in params:
            atr_span = params[symbol][0]
            atr_ratio = params[symbol][1]
            maxl = params[symbol][2]
            entry_dayofweek = params[symbol][3]
        backtest_run(symbol
                     , ashi
                     , start_date
                     , end_date
