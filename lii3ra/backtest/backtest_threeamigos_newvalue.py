...
from donkatsu.entry_strategy.three_amigos import ThreeAmigos
...
def backtest_run(symbol
    ...
    try:
        ...
        adx = AverageDirectionalIndex(ohlcv, adx_span)
        rsi = RelativeStrengthIndex(ohlcv, rsi_span)
        open_title = f"ThreeAmigos[{adx_span:.0f},{adx_threshold:.0f},{rsi_span:.0f},{rsi_threshold:.0f},{lookback1:.0f},{lookback2:.0f}]"
        close_title = f"NewValue"
        open_strategy = ThreeAmigos(open_title, ohlcv, adx, adx_threshold, rsi, rsi_threshold, lookback1, lookback2)
...
def backtest(symbol, ashi, start_date, end_date, brute_force=False):
    ...
    if brute_force:
        ...
        # symbolごとのparameter
        if symbol in params:
            adx_span = params[symbol][0]
            adx_threshold = params[symbol][1]
            rsi_span = params[symbol][2]
            rsi_threshold = params[symbol][3]
            lookback1 = params[symbol][4]
            lookback2 = params[symbol][5]
        backtest_run(symbol
                     , ashi
                     , start_date
                     , end_date
